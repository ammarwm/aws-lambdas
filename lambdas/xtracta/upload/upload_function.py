...
def handler(event, context=None):
    ...
    LOGGER.info(event)
    result = {'status':'400', 'messages':'Error'}
    ...
    if workflow_id:
        xtracta_response = upload_document_to_xtracta(workflow_id, document_file)
    else:
        logging.error("No workflow_id for master_document_id: %s", master_document_id)
        return result
    if xtracta_response['status'] == '200':
        insert_document(xtracta_response['document_id'], lendi_document_id,\
                        workflow_id, 'preprocessing')
    else:
        logging.error("No xtracta_document_id for %s", xtracta_response['message'])
        return xtracta_response
    return xtracta_response
...
def insert_document(xtracta_document_id, lendi_document_id, workflow_id, status):
    ...
    result = {'status':'400', 'message':'Error to connect to db', 'document_id':''}
    try:
        conn = dbconnector.connect(**DATABASES['lendi_ai'])
    except dbconnector.Error:
        LOGGER.exception("Error update_document_status(), db connection error")
        return result
    cur = conn.cursor()
    command = SQLQUERIES['select_xtracta_document_id'].format(xtracta_document_id)
    try:
        cur.execute(command)
        result = cur.fetchone()
        if result:
            ...
            if not lendi_document_id_:
                command = SQLQUERIES['update_lendi_document_id'].format(lendi_document_id)
                cur.execute(command)
        else:
            command = SQLQUERIES['insert_new_document'].format(xtracta_document_id,\
                        lendi_document_id, status)
            cur.execute(command)
    except dbconnector.Error:
...
def upload_document_to_xtracta(workflow, document_file):
    ...
    if response_content['xml']['status'] == '200':
        return response_content['xml']
    else:
        LOGGER.exception("upload_document_to_xtracta() %s", response_content['xml']['message'])
    return response_content['xml']
